...
function [Store,M]=MH5N1(nPop,Max_iter,lb,ub,dim,CostFunction,nObj)
...
ArchiveMaxSize=300; % Số lượng tối đa lưu lại 
...
%     popPosition(changeRows) = pmp2(changeRows);
    popPosition = CheckBoundary(popPosition, lb, ub);
...
[minF1,n_minF1]=min(Archive_F(:,1));
[minF2,n_minF2]=min(Archive_F(:,2));
M.XminF1 = Archive_X(n_minF1,:);
M.XminF2 = Archive_X(n_minF2,:);
M.minF1 = Archive_F(n_minF1,:);
M.minF2 = Archive_F(n_minF2,:);
M.Archive_min = minF1 + minF2;
end
