...
function [GlobalBest_Cost,GlobalBest_Position,BestCosts,Store]=AOA(nPop,MaxIt,VarMin,VarMax,nVar,CostFunction)
% display('AOA Working');
...
for i=1:size(X,1)
    Ffun(1,i)=CostFunction(X(i,:));  %Calculate the fitness values of solutions
...
while iter<MaxIt+1  %Main loop
    ...
    for i=1:size(X,1)   % if each of the UB and LB has a just value 
        ...
        Ffun_new(1,i)=CostFunction(Xnew(i,:));  % calculate Fitness function 
